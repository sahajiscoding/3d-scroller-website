...
function LoadingSkeleton({ visible }: { visible: boolean }) {
  return (
    ...
    >
      ...
      <div className="animate-pulse-soft relative h-40 w-40 rounded-full bg-[radial-gradient(circle_at_35%_30%,#46e3ff_0%,#5b8cff_45%,rgba(5,5,8,0)_72%)] blur-[2px]" />
  ...
  return (
    <div className="absolute inset-0 z-10 overflow-hidden bg-ink" aria-hidden="true">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        <div className="animate-pulse-soft h-72 w-72 rounded-full bg-[radial-gradient(circle_at_35%_30%,#46e3ff_0%,#5b8cff_50%,rgba(5,5,8,0)_74%)] blur-[6px]" />
        <div className="animate-spin-slow absolute inset-[-3rem] rounded-full border border-dashed border-volt/30" />
        <div className="absolute inset-[1.5rem] rounded-full border border-aqua/20" />
      </div>
    </div>
  );
}
